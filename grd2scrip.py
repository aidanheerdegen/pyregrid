...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert grid file to SCIP format")
    ...
    for input in args.inputs:
        ...
        if input.endswith(suffix):
            output = input[:-len(suffix)] + '_scrip.nc'
        else:
            output = input + '_scrip.nc'

        if verbose:
            print("Input file: {}".format(input))
            print("SCRIP output file: {}".format(output))

        # grid = MomGrid.fromfile(input)
        grid = get_grid(input)
        grid.write_scrip(output)
        # grid.write_test_scrip(output)
